...
```{r, message=FALSE, warning=FALSE, error=FALSE}
var_select <- function(start, select) {
...
```{r, message=FALSE, warning=FALSE, error=FALSE}
model_tune <- function(Pmax, Qmax, start, select) {
  ...
  var_subset <- var_select(start, select)
...
start <- Sys.time()
tuned <- model_tune(1, 2, start, 3); tuned
```
