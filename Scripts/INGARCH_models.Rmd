...
```{r}
cross_valid <- function(resp, p, q, CV) {
  # define empty vector to store MSE values
  mses <- rep(NA, CV)
  
  for (i in 1:CV) {
    # choose trials randomly to serve as training and validation sets
    trials <- sample(1:80, 2, replace = FALSE)
    # collect data frames
    dfs <- gen_df(trials[1], trials[2], resp)
    train_df <- dfs[['train_df']]
    valid_df <- dfs[['valid_df']]
    # filter validation set to first 100 entries for time constraint
    valid_df <- valid_df[1:100, ]
    valid_pred <- dfs[['valid_pred']]
    valid_pred <- valid_pred[1:100,1]
    remove(dfs)
    ...
    tryCatch(
      {
        ...
        if (mse > 10) {
          print(mse)
          print(pred[['pred']][which(pred[['pred']] > 10)])
        }
...
for (resp in 'CA1.1') {
  ...
  for (p in 1:Pmax) {
    for (q in 1:Qmax) {
      tuning$mse[which(tuning$p == p & tuning$q == q)] <- cross_valid(resp, 
                                                                      p, q, CV)
    }
    print(paste('Done with p =', p, 'in', Sys.time() - start, 'minutes...'))
  }
