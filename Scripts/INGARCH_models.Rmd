    ...
    tryCatch(
      {
        ...
        temp <- summary(model)
        est <- temp$coefficients$Estimate[-c(1:3, 25, 26)]
...
getCoefs <- function(predictors, dist) {
  ...
  for (resp in resp_names) {
    if (dist == 'poisson') {
      model <- readRDS(paste('../Models/INGARCH_Pois/model_', resp, sep = ''))
    } else {
      model <- readRDS(paste('../Models/INGARCH_NBinom/model_', resp, sep = ''))
    }
    
    model_sum <- summary(model)
    est <- model_sum$coefficients$Estimate[-c(1, nrow(model_sum$coefficients))]
  
    nms <- rownames(model_sum$coefficients)[which(
      !grepl('var', rownames(model_sum$coefficients)) & 
        !grepl('Intercept', rownames(model_sum$coefficients)) & 
        !grepl('sigma', rownames(model_sum$coefficients)) &
        !grepl('trial', rownames(model_sum$coefficients)))]
  ...
  coefs %>%
    saveRDS('../Optimize/INGARCH_Pois/coefs.RData')
...
predictors <- var_select(10, start, dist)
...
Collect estimated coefficients from optimal poisson models:
```{r}
coefs <- getCoefs(predictors, dist)
```

Plot coefficient estimates from poisson models:
```{r}
coefs %>%
  ...
  labs(title='Poisson INGARCH Coefficient Estimates', 
...
Collect estimated coefficients from optimal negative binomial models:
```{r}
coefs <- getCoefs(predictors, dist)
```

Plot coefficient estimates from negative binomial models:
```{r}
coefs %>%
  pivot_longer(!resp, names_to = 'pred', values_to = 'est') %>%
  ggplot() +
  ...
  scale_fill_continuous() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  labs(title='Negative Binomial INGARCH Coefficient Estimates', 
       x='Predictor', y='Response', fill='')
```
...
if (dist == 'poisson') {
  readRDS(paste('../Models/INGARCH_Pois/model_', resp, sep = '')) %>% 
    plot()
} else {
  readRDS(paste('../Models/INGARCH_NBinom/model_', resp, sep = '')) %>% 
    plot()
}
```
