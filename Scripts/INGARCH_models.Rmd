...
```{r, message=FALSE, warning=FALSE, error=FALSE}
var_select <- function(select, start) {
...
```{r, message=FALSE, warning=FALSE, error=FALSE}
model_tune <- function(Pmax, Qmax, predictors, start) {
  ...
  for (resp in resp_names) {
    ...
    for (p in 1:Pmax) {
      for (q in 1:Qmax) {
        ...
        tuning$mse[which(tuning$p == p & tuning$q == q)] <- cross_valid(
          as.vector(predictors[[resp]]), resp, p, q)
      }
    }
    ...
    model_tune_mses[[resp]] <<- tuning$mses
...
tuned <- model_tune(2, 2, predictors, start)
```

```{r}
start <- Sys.time()
tuned <- model_tune(10, 10, predictors, start)
```
...
predictors %>%
  saveRDS('../Temp/predictors.RData')

tuned %>%
  saveRDS('../Temp/tuned.RData')
```
