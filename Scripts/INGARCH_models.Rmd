...
\sum_{k=1}^q a_k \nu_{t-k} + \boldsymbol{\eta}^{T} \boldsymbol{X}_t \\
    ...
    tryCatch(
      {
        ...
        if (dist == 'poisson') {
          model %>%
            saveRDS(paste('../Models/INGARCH_Pois/model_', resp, '.rds', 
                          sep = ''))
        } else {
          model %>%
            saveRDS(paste('../Models/INGARCH_NBinom/model_', resp, '.rds', 
                          sep = ''))    
        }
      }, 
      error = function(err) {
...
getCoefs <- function(predictors, dist) {
  ...
  for (resp in resp_names) {
    ...
    tryCatch(
      {
        if (dist == 'poisson') {
          model <- readRDS(paste('../Models/INGARCH_Pois/model_', resp, '.rds',
                                 sep = ''))
        } else {
          model <- readRDS(paste('../Models/INGARCH_NBinom/model_', resp, 
                                 '.rds', sep = ''))
        }
      }, 
      error = function(err) {
  ...
  if (dist == 'poisson') {
    ...
    coefs %>%
      saveRDS('../Optimize/INGARCH_NBinom/coefs.RData')
  }
...
test_mses %>%
  saveRDS('../Optimize/INGARCH_Pois/test_mses.RData')
...
if (dist == 'poisson') {
  ...
  readRDS(paste('../Models/INGARCH_NBinom/model_', resp, '.rds', sep = '')) %>% 
    plot()
}
...
if (dist == 'poisson') {
  ...
  predictors <- readRDS('../Optimize/INGARCH_NBinom/predictors.RData')
  tuned <- readRDS('../Optimize/INGARCH_NBinom/tuned.RData')
}
