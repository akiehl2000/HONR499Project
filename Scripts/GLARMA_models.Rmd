...
library(tidyverse)
library(tscount) # GLARMA model implementation
    ...
    tryCatch(
      {
        ...
        if (dist == 'poisson') {
          model %>%
            saveRDS(paste('../Models/GLARMA_Pois/model_', resp, '.rds', 
                          sep = ''))
        } else {
          model %>%
            saveRDS(paste('../Models/GLARMA_NBinom/model_', resp, '.rds', 
                          sep = ''))    
        }
      }, 
      error = function(err) {
...
getCoefs <- function(predictors, dist) {
  ...
  for (resp in resp_names) {
    ...
    tryCatch(
      {
        if (dist == 'poisson') {
          model <- readRDS(paste('../Models/GLARMA_Pois/model_', resp, '.rds',
                                 sep = ''))
        } else {
          model <- readRDS(paste('../Models/GLARMA_NBinom/model_', resp, 
                                 '.rds', sep = ''))
        }
      }, 
      error = function(err) {
  ...
  if (dist == 'poisson') {
    coefs %>%
      saveRDS('../Optimize/GLARMA_Pois/coefs.RData')
  } else {
    coefs %>%
      saveRDS('../Optimize/GLARMA_NBinom/coefs.RData')
  }
...
```{r results-poisson}
predictors %>%
  saveRDS('../Optimize/GLARMA_Pois/predictors.RData')

tuned %>%
  saveRDS('../Optimize/GLARMA_Pois/tuned.RData')

var_select_mses %>%
  saveRDS('../Optimize/GLARMA_Pois/varSelectMSE.RData')

model_tune_mses %>%
  saveRDS('../Optimize/GLARMA_Pois/modelTuneMSE.RData')
```
...
test_mses %>%
  saveRDS('../Optimize/GLARMA_Pois/test_mses.RData')
...
coefs %>%
  ...
  labs(title='Poisson GLARMA Coefficient Estimates', 
...
```{r results-nbinom}
predictors %>%
  saveRDS('../Optimize/GLARMA_NBinom/predictors.RData')

tuned %>%
  saveRDS('../Optimize/GLARMA_NBinom/tuned.RData')

var_select_mses %>%
  saveRDS('../Optimize/GLARMA_NBinom/varSelectMSE.RData')

model_tune_mses %>%
  saveRDS('../Optimize/GLARMA_NBinom/modelTuneMSE.RData')
```
...
test_mses %>%
  saveRDS('../Optimize/GLARMA_NBinom/test_mses.RData')
...
coefs %>%
  ...
  scale_fill_continuous() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  labs(title='Negative Binomial GLARMA Coefficient Estimates', 
       x='Predictor', y='Response', fill='')
```
