...
cross_valid <- function(pred_list, resp_list, p, dist) {
  ...
  for (i in 1:2) {
    ...
    tryCatch(
      ...
      error = function(err) {
        print(paste('Model fitting error on (', p, '): ', err, 
                    sep = ''))
      }
    )
    ...
    tryCatch(
      ...
      error = function(err) {
        print(paste('Model evaluation error on (', p, '): ', err, 
                    sep = ''))
      }
      ...
      tryCatch(
        {
          ...
          mses[i] <- cross_valid(preds, resp, 1, dist)
        }, 
        error = function(err) {
          print(paste('Error:', err))
        }
...
fitOpt <- function(predictors, tuned, dist) {
  ...
  for (resp in c('CA1.12', 'CA1.13', 'CA1.14', 'CA1.15')) {
    ...
    tryCatch(
      {
        ...
        if (dist == 'poisson') {
          model %>%
            saveRDS(paste('../Models/GLAR_Pois/model_', resp, '.rds', sep = ''))
        } else {
          model %>%
            saveRDS(paste('../Models/GLAR_NBinom/model_', resp, '.rds', sep = ''))    
        }
      }, error = function(err) {
        print(paste('Error fitting optimized model for', resp))
      }
    )
  }
...
getCoefs <- function(predictors, dist) {
  ...
  for (resp in resp_names) {
    if (dist == 'poisson') {
      ...
      model <- readRDS(paste('../Models/GLAR_NBinom/model_', resp, '.rds', 
                             sep = ''))
    }
    ...
    tryCatch (
      {
        ...
        est <- model_sum$Estimate[-1]

        nms <- rownames(model_sum)[which(
          !grepl('var', rownames(model_sum)) & 
          !grepl('Intercept', rownames(model_sum)) & 
          !grepl('sigma', rownames(model_sum)) &
          !grepl('trial', rownames(model_sum)))]
        nms <- c(nms, predictors[[resp]], 'trial')

        coefs[which(coefs$resp == resp), nms] <- est
      }, error = function(err) {
        print(paste('Error collecting coefficients for', resp))
      }
    )
