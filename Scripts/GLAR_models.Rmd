...
fitOpt <- function(predictors, tuned, dist) {
  ...
  for (resp in resp_names) {
    ...
    if (dist == 'poisson') {
      model %>%
        saveRDS(paste('../Models/GLAR_Pois/model_', resp, sep = ''))
    } else {
      model %>%
        saveRDS(paste('../Models/GLAR_NBinom/model_', resp, sep = ''))    
    }
...
getCoefs <- function(predictors, dist) {
  ...
  for (resp in resp_names) {
    if (dist == 'poisson') {
      model <- readRDS(paste('../Models/GLAR_Pois/model_', resp, sep = ''))
    } else {
      model <- readRDS(paste('../Models/GLAR_NBinom/model_', resp, sep = ''))
    }
