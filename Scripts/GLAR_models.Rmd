...
The univariate GLAR(p) model fit here using the `tsglm` function in the 
`tscount` package is specified according to Davis, et al. like:
$$
\begin{aligned}
X_t|F_{t-1} &\sim Poisson(\lambda_t) \\
\lambda_t &= d + \sum_{j=1}^p b_{j} log(X_{t-j})
\end{aligned}
$$
The marginal mean of the response poisson distribution is described in a
manner resembling a simple autoregressive model, with the `p` parameter defining
the extent of the autoregressive lag.  
...
cross_valid <- function(pred_list, resp_list, p) {
...
Perform variable selection on the GLAR model using a forward selection 
...
Tune the GLAR model by looping through parameter values and calling the 
`cross_valid` function above:
```{r, message=FALSE, warning=FALSE, error=FALSE}
model_tune <- function(Pmax, predictors, start) {
  ...
  for (resp in resp_names) {
    ...
    for (p in 1:Pmax) {
      ...
      tuning$mse[which(tuning$p == p)] <- cross_valid(
        as.vector(predictors[[resp]]), resp, p)
    }
...
tuned <- model_tune(2, predictors, start)
```
...
coefs %>%
  ...
  labs(title='GLAR Coefficient Estimates', 
